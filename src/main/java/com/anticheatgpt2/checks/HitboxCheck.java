...
public class HitboxCheck implements Listener {
    ...
    private static final double MAX_HITBOX_DISTANCE = 3.0; // Максимальная допустимая дистанция удара (в блоках)

    // Проверка на манипуляции с hitbox
    @EventHandler
    public void onPlayerHit(EntityDamageByEntityEvent event) {
        ...
        if (event.getEntity() instanceof Player) {
            ...
            if (attacker instanceof Player) {
                ...
                if (distance > MAX_HITBOX_DISTANCE) {
                    // Подозрительный удар с дальностью больше разрешенной
                    event.setCancelled(true); // Отменяем повреждение
                    attackerPlayer.sendMessage("§cПодозрительный удар! Возможно, манипуляции с hitbox.");
                }
            }
        }
    }
